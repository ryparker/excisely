/**
 * Diagnostic script: tests the local Tesseract OCR pipeline against all
 * organized test label images and reports PASS/FAIL for each field.
 *
 * Run: npx tsx scripts/diagnose-local-pipeline.ts
 */
...
async function main() {
  ...
  console.log('  LOCAL PIPELINE DIAGNOSTIC — Tesseract OCR + Text Search')
  console.log('='.repeat(80))
  console.log()
  ...
  for (const label of TEST_LABELS) {
    ...
    try {
      ...
      console.log(
        `  OCR: ${result.metrics.ocrTimeMs}ms | Classification: ${result.metrics.classificationTimeMs}ms | Total: ${elapsedMs}ms`,
      )
      console.log()
