...
export function DeadlineDisplay({
  ...
  const colors =
    variant === 'applicant' ? APPLICANT_URGENCY_COLORS : SPECIALIST_URGENCY_COLORS

  if (info.urgency === 'expired') {
    return <span className={colors.expired}>Expired</span>
  }

  const suffix = variant === 'applicant' ? 'd remaining' : ` day${info.daysRemaining !== 1 ? 's' : ''}`

  return (
    <span className={colors[info.urgency]}>
      {info.daysRemaining}{suffix}
    </span>
  )
}
