...
// Use applicant auth state
test.use({ storageState: 'e2e/.auth/applicant.json' })
...
test.describe('Submit Labels', () => {
  ...
  for (let i = 0; i < availableLabels.length; i++) {
    ...
    test(`submit #${i + 1} ${label.brandName}`, async ({ page }) => {
      ...
      if (label.classType) {
        await page.getByLabel(/class\/type designation/i).fill(label.classType)
      }
