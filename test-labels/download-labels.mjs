...
const colas = [
  // Whiskey/Bourbon
  { ttbid: '25085001000195', name: 'knob-creek', category: 'whiskey', prefix: 'bourbon' },
  { ttbid: '25093001000227', name: 'bulleit-rye', category: 'whiskey', prefix: 'rye' },
  { ttbid: '25136001000083', name: 'bulleit-old-fashioned', category: 'whiskey', prefix: 'whiskey' },
  { ttbid: '25150001000553', name: 'bulleit-bourbon-10yr', category: 'whiskey', prefix: 'bourbon' },
  // Wine
  { ttbid: '24001001000011', name: 'cooper-ridge-malbec', category: 'wine', prefix: 'wine' },
  { ttbid: '24001001000015', name: 'domaine-montredon', category: 'wine', prefix: 'wine' },
  { ttbid: '24001001000016', name: 'dashfire-cocktail', category: 'wine', prefix: 'wine' },
];
...
async function main() {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      await page.goto('https://ttbonline.gov/colasonline/publicSearchColasBasic.do', { waitUntil: 'load', timeout: 15000 });
      console.log('Session established!');
      break;
    } catch (e) {
      console.log(`  Attempt ${attempt + 1} failed, retrying in ${(attempt + 1) * 3}s...`);
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        for (const img of images) {
          ...
          let type = 'other';
          if (img.alt.includes('front') || img.alt.includes('Brand')) type = 'front';
          else if (img.alt.includes('Back')) type = 'back';
          else if (img.alt.includes('Neck')) type = 'neck';
          else if (img.alt.includes('Strip')) type = 'strip';

          const sameTypeCount = images.filter(i2 => i2.index <= img.index && i2.alt === img.alt).length;
          ...
          try {
            ...
            console.log(`  Saved: ${filename} (${img.naturalWidth}x${img.naturalHeight})`);
            saved.push(filename);
          } catch (e) {
  ...
  async function searchAndDownload(searchTerm, category, prefix) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        await page.waitForURL('**/publicSearchColasBasicProcess*', { timeout: 15000 });
        await sleep(2000);
        ...
        for (const ttbid of ttbids.slice(0, 3)) {
          const saved = await downloadLabelsFromCola(ttbid, searchTerm.toLowerCase().replace(/\s+/g, '-'), category, prefix);
  ...
  for (const cola of colas) {
    await downloadLabelsFromCola(cola.ttbid, cola.name, cola.category, cola.prefix);
  }
